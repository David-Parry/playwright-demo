...
const SupportSection = ({ icon, title, description, buttonText, onClick }) => (
  <section className="text-muted text-start w-100">
    <section className="icon-section d-flex p-2">
      {icon}
      <p className="text-capitalize px-3 text-dark">{title}</p>
    </section>
    {description}
...
const Settings = () => {
  ...
  // Navigation handlers
  const navigateHome = () => navigate("./weather");
  const navigateToProject = () => navigate("https://github.com/Adedoyin-Emmanuel/react-weather-app");
  const navigateToGithub = () => navigate("https://github.com/Adedoyin-Emmanuel");
